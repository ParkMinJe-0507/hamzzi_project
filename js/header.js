...
class HeaderComponent extends HTMLElement {
  constructor() {
    super();
    // 로그아웃 이벤트 핸들러를 클래스 속성으로 미리 정의합니다.
      ...
      <header class="main-header">
        <div class="header-inner">
          ...
          <div class="header-actions">
            ...
            <a href="/pages/login/sign_in/index.html" id="auth-link" class="login-link">로그인</a>
  ...
  // 로그인 상태를 감지하고 UI를 업데이트하는 함수
  updateLoginStatus() {
    onAuthStateChanged(auth, (user) => {
      const authLink = this.querySelector('#auth-link');
      const welcomeMsg = this.querySelector('#welcome-msg');
      
      if (!authLink || !welcomeMsg) return;

      if (user) {
        // 사용자가 로그인한 경우
        welcomeMsg.textContent = `${user.email.split('@')[0]}님`;
        authLink.textContent = '로그아웃';
        authLink.href = '#';
        authLink.removeEventListener('click', this.handleLogout);
        authLink.addEventListener('click', this.handleLogout);
      } else {
        // 사용자가 로그아웃한 경우
        welcomeMsg.textContent = '';
        authLink.textContent = '로그인';
        authLink.href = '/pages/login/sign_in/index.html';
        authLink.removeEventListener('click', this.handleLogout);
      }
    });
  }
  ...
  addEventListeners() {
    ...
    if (!searchBtn || !closeSearchBtn || !searchOverlay || !toggleBtn || !searchInput) return;
